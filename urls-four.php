...
// hyderabad-real-estate/apartment-flat-sale/3bhk
$url1 = $_GET['url1'];
$url2 = $_GET['url2'];
$url3 = $_GET['url3'];

// $url1='hyderabad-real-estate';
// $url2='apartment-flat-sale';
// $url3='3bhk';
...
if (!empty($url1 && $url2 && $url3)) {
	// ---  start city and property type and bed room ---
	$countFourUrlProjects = mysqli_query($db_connect, "
  SELECT count(pd.project_id) AS countProjects
  FROM project_detail AS pd,city AS ct, property_types AS pt, bed_rooms AS br, unit_configuration AS uc
  WHERE pd.city_id=ct.city_id AND pd.property_types_id=pt.property_types_id AND br.bed_rooms_id=uc.unit_types_id AND pd.project_id=uc.project_id AND ct.city_url='$url1' AND pt.property_types_url='$url2' AND br.bed_rooms_url='$url3' AND pd.builders_id = '$builders_id' ");
	while ($row = mysqli_fetch_array($countFourUrlProjects)) {
		$noOfProjects = $row['countProjects'];
		$totalPages = ceil($noOfProjects / $limit_per_page);
		$noOfCompletedProjects = $row['countProjects'];
	}
	$selectfourUrlProjects = mysqli_query($db_connect, "
    SELECT * FROM project_detail AS pd,builders_list AS bl,country AS cr,state AS st,city AS ct,area AS ar,unit_types AS ut,property_types AS pt,property_status AS ps, unit_configuration AS uc, bed_rooms AS br
    WHERE pd.builders_id=bl.builders_id AND pd.country_id=cr.country_id AND pd.state_id=st.state_id AND pd.city_id=ct.city_id AND pd.area_id=ar.area_id AND pd.unit_types_id=ut.unit_types_id AND pd.property_types_id=pt.property_types_id AND pd.property_status_id=ps.property_status_id AND pd.project_id=uc.project_id AND uc.unit_types_id=br.bed_rooms_id AND ct.city_url='$url1' AND pt.property_types_url='$url2' AND br.bed_rooms_url='$url3' AND pd.builders_id = '$builders_id' ORDER BY project_name ASC LIMIT $starting_from , $limit_per_page ");

	if (mysqli_num_rows($selectfourUrlProjects) >= 1) {
		$resultArray = array();
		while ($row = mysqli_fetch_array($selectfourUrlProjects)) {
			$bredCrumbTitle = $row['bed_rooms_name'] . ' ' . $row['property_types_name'] . ' in ' . $row['city_name'];
			$heroTitle = $row['bed_rooms_name'] . ' ' . $row['property_types_name'] . ' in ' . $row['city_name'];
			$cityName = $row['city_name'];
			$resultArray[] = $row;
		}

		// ---  start city and property type and bedrooms ---
      ...
      WHERE pd.builders_id=bl.builders_id AND pd.state_id=st.state_id AND pd.city_id=ct.city_id AND ct.city_url='$url1' GROUP BY bl.builders_name ");
      ...
      WHERE pd.project_id=uc.project_id AND br.bed_rooms_id=uc.unit_types_id AND pd.state_id=st.state_id AND pd.city_id=ct.city_id AND ct.city_url='$url1' GROUP BY br.bed_rooms_name ORDER BY bed_rooms_id ASC");
		...
		// ---  end city and property type and bedrooms ---
	}
	if (mysqli_num_rows($selectfourUrlProjects) <= 0) {
		$bredCrumbTitle = 'No Results Found';
		$heroTitle = 'No Results Found';
	}
	// ---  end city and property type and bed room ---
}
